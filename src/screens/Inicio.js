    ...
    render() {
        return (
            <View style={styles.container}>
                <Header image={logo_icon} texto={"Página Inicial"} icon={"search"} onPress={() => Alert.alert("Teste", "teste")} />
                <FlatList
                    data={this.state.ideias}
                    keyExtractor={item => `${item.id_ideia}`}
                    renderItem={({ item }) =>
                        <Ideia key={item.id_ideia} onPressCurtir={() => this.curtirIdeia(item.id_ideia)} onPressAutor={() => this.infoAutor(item.id_usuario)}
